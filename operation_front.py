...
class VisualControl(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_reset_view = QPushButton("Reset Camera")
...
class RobotFrontEnd(QWidget):
    ...
    def __init__(self, injector_control, config_data=None):
        ...
        self._startup_updates_frozen = False
        self._viz_ready_emitted = False
        self.cap = None
    ...
    def _init_visualization(self):
        ...
        camera_cfg = self.config_data.get("VISUALIZATION_CAMERA", {})
        self.viz_renderer = Visualizer3D(self.ui.widget_open3d, camera_config=camera_cfg)
        ...
        self.viz_renderer.first_frame_ready.connect(self._on_visualizer_first_frame_ready)

    @Slot()
    def _on_visualizer_first_frame_ready(self):
        if self._viz_ready_emitted:
            return

        self._viz_ready_emitted = True
        self.visualization_ready.emit()

    def set_startup_updates_frozen(self, frozen):
        """Freeze non-critical UI refreshes during startup transition."""
        self._startup_updates_frozen = frozen

    def set_visualization_pending_hint(self, pending):
        """Show a subtle hint when renderer takes longer than startup timeout."""
        if pending:
            self.ui.label_safe_hint.setText("3D renderer is still initializing...")
            self.ui.label_safe_hint.setVisible(True)
        else:
            self.ui.label_safe_hint.setText("Please switch to WORK POSITION to operate robot")
    ...
    def _on_tick(self):
        ...
        if not self.is_running or self._startup_updates_frozen:
            return
