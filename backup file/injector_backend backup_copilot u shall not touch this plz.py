...
class InjectorBackend(QObject):
    ...
    def connect_zaber(self):
        ...
        try:
            ...
            if len(detected_devices) > 1:
                ...
                # Start polling
                refresh_interval = session_globals.get_setting("REFRESH_RATE_MS", 100)
                self.poll_timer.start(refresh_interval)
            else:
    ...
    @Slot()
    def sethome(self):
        """Homes linear axes for the injector."""
        if not self.axes:
        ...
        try:
            ...
            while self.device.all_axes.is_busy():
                time.sleep(0.01)
    ...
    def setangle(self, to_safe: bool):
        """Sets the Rz axis to a safe (home) or working (90 deg) angle."""
        ...
        try:
            ...
            if to_safe:
                ...
                axis.move_absolute(90.0, unit=Units.ANGLE_DEGREES, wait_until_idle=True)
    ...
    def move_relative(self, x: float, y: float, z: float):
        """Linear relative movement for the injector."""
        if not self.axes:
    ...
    def poll_positions(self):
        """Callback for periodic status monitoring."""
        ...
        try:
            ...
            session_globals.set_setting("current_pos_injector", positions)
            self.position_updated.emit(positions)
        except Exception as e:
            # Silence polling errors unless they are critical
            pass

    def _check_range(self, dx: float, dy: float, dz: float) -> bool:
        """Internal bounds checking."""
        current_pos = session_globals.get_setting("current_pos_injector", {'X': 0, 'Y': 0, 'Z': 0})
        
        new_x = current_pos.get('X', 0) + dx
        new_y = current_pos.get('Y', 0) + dy
        new_z = current_pos.get('Z', 0) + dz
